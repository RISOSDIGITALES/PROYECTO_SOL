...
// LA MISMA animación que PoppableBubbles.jsx (.bubble54-amb.bursting) — no una
...
// recibe una clase de pulso (`.bubble54-burstkeep`, en theme.css) que anima
...
// `.bubble54-amb.bursting`. El destello es el mismo ::before de siempre (mismo
// keyframe bubble54-flash, solo centrado con --burst-size en vez de
...
// del mismo contenedor — con el mismo `.bubble54-shard`/`bubble54-shard-fly`, y
...
export function burst(el) {
  ...
  el.classList.remove("bubble54-burstkeep");
  el.querySelectorAll(".bubble54-shard").forEach((s) => s.remove());
  void el.offsetWidth;
  el.classList.add("bubble54-burstkeep");
  el.addEventListener("animationend", () => el.classList.remove("bubble54-burstkeep"), { once: true });
  ...
  setTimeout(() => el.classList.remove("bubble54-burstkeep"), 500);

  shardOffsets(size).forEach((s) => {
    const shard = document.createElement("span");
    shard.className = "bubble54-shard";
  ...
  // bubble54-shard-fly dura 0.65s — con margen de sobra, mismo motivo que el
  // setTimeout de arriba.
  setTimeout(() => el.querySelectorAll(".bubble54-shard").forEach((s) => s.remove()), 700);
}
