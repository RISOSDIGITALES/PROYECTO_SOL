...
// agencia). El CSS real (.bubble54-amb, .bursting, .bubble54-shard, etc.) vive
  ...
  return bubbles.map((b) => {
    ...
    return (
      <Fragment key={b.id}>
        <span
          aria-hidden="true"
          className={`bubble54-amb poppable ${b.hue || ""} ${shards ? "bursting" : ""}`}
          style={posStyle}
        ...
        {shards && (
          <span className="bubble54-shardfield" style={posStyle} aria-hidden="true">
            {shards.map((s, i) => (
              <span key={i} className="bubble54-shard" style={{ "--tx": `${s.tx}px`, "--ty": `${s.ty}px` }} />
            ))}
          </span>
        )}
      </Fragment>
