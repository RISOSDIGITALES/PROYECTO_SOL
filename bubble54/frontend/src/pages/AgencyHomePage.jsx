      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "36px 40px" }}>
        ...
        <div className="bubble54-cols-460" style={colsStyle}>
          ...
          <div style={{ display: "grid", gap: 20, alignContent: "start" }}>
            {!loading && !allDone && (
              <SectionCard title="Primeros pasos" hue={HUE.inicio} icon="home">
                <div style={{ display: "grid", gap: 8 }}>
                  {steps.map((s) => (
                    <Link key={s.label} to={s.to} className="bubble54-steprow" style={stepRowStyle}>
            ...
            {!loading && allDone && (
              <div className="bubble54-panel" style={{ padding: 22, display: "flex", alignItems: "center", gap: 14, borderLeft: `3px solid ${HUE.inicio}` }}>
          ...
          <div style={{ display: "grid", gap: 20, alignContent: "start" }}>
            ...
            <SectionCard title={`Negocios (${loading ? "…" : businesses.length})`} hue={HUE.negocios} icon="briefcase" linkTo="/agencia/negocios" linkLabel="Ver todos →">
              {loading ? (
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {businesses.map((b) => (
                    <Link key={b.id} to={`/agencia/negocios/${b.id}`} className="bubble54-steprow" style={businessRowStyle}>
...
function SectionCard({ title, hue, icon, linkTo, linkLabel, children }) {
  return (
    <div className="bubble54-panel" style={{ padding: 22, borderLeft: `3px solid ${hue}` }}>
