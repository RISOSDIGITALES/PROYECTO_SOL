"""Worker de LiveKit Agents para Bubble54 — reemplaza la dependencia de VAPI.

Investigado el 2026-08-29 (ver README de bubble54/): ni Pipecat ni LiveKit Agents
...
Bubble54) ES esa capa. Este worker solo LEE esa config al arrancar una llamada
...
- ai_provider "anthropic"/"gemini": el catálogo de Bubble54 los permite para
...
logger = logging.getLogger("bubble54-worker")

BUBBLE54_API_BASE = os.environ.get("BUBBLE54_API_BASE", "http://localhost:8000")
...
async def fetch_bot_config(business_id: int | None = None, phone_number: str | None = None) -> dict:
    """Trae la BotConfig real desde el backend de Bubble54 — nunca se inventa
    ...
    async with httpx.AsyncClient(base_url=BUBBLE54_API_BASE, timeout=10.0) as client:
...
def build_instructions(config: dict) -> str:
    ...
    (a qué se dedica, cuándo atiende, qué vende) si lo cargó en Bubble54 —
