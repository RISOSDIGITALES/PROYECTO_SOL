# Bubble54 (nombre provisorio — pendiente de decidir el nombre real)
...
- **No dependemos de VAPI (ni de ningún otro orquestador todo-en-uno).**
  ...
  Agents** (`bubble54/worker/`, proceso Python separado del backend), armado
...
## Worker de LiveKit Agents (`bubble54/worker/`)
...
2. Llama a `GET /worker/bot-config/{business_id}` (o `/by-phone/{numero}`) del
   backend de Bubble54, autenticado con un secreto compartido (`WORKER_SECRET`,
...
esa máquina): levantalo en otro puerto libre y creá `bubble54/frontend/.env`
...
# bubble54.db, y de la carpeta real de uploads/ vía un fixture autouse), cubren
...
- [x] Worker de LiveKit Agents (`bubble54/worker/agent.py`) — arma el pipeline de
...
Wrapper) que le agregan esa capa encima. Eso confirma que el hueco que Bubble54
...
Bubble54 es telefonía real, y LiveKit tiene reglas de SIP dispatch de primera
