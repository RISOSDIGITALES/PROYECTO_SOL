...
- [ ] **tipo_cajon no se actualiza al cambiar de opinión a mitad de conversación** (hallazgo del 2026-07-10, ítem 108): si el cliente dice "jaula está bien" después de que Marco ya había seteado "cajones_cerrados", el mensaje reconoce el cambio pero el dato guardado se queda con el valor viejo — podría generar una cotización con el tipo de cajón incorrecto. Sin investigar todavía.
...
**Nota aparte, no relacionada al login:** al revisar el perfil de las dos empresas se notó que los `timezone` devueltos por el login parecen estar cruzados — Crating Express (Miami) figura con `America/Managua` y Orison (Managua) figura con `America/Chicago`. Podría ser el mismo tipo de bug que el pendiente de "huso horario por ciudad" del Éxodo del 08-jul — sin confirmar todavía, pendiente de mirar con más calma.

115. **Revisión de 3 pendientes del Éxodo con el token de sesión real** (2026-07-13, misma tarde): (1) **Huso horario por ciudad — confirmado el bug, no es solo una sospecha**: `GET /api/auth/me` devuelve el campo `timezone` por empresa, y sigue cruzado — Crating Express (Miami) tiene `America/Managua`, Orison Managua tiene `America/Chicago`. La funcionalidad de huso horario por empresa ya existe (avance real sobre el "Londres hardcodeado" del Éxodo original), pero los valores de estas dos empresas están invertidos entre sí. (2) **Registro de agentes / Distribution AI en el panel — no se pudo verificar por API**: se probaron 8 nombres de ruta más (`/api/agents`, `/api/integrations`, `/api/webhooks`, `/api/company-agents`, etc.) con el token de sesión real, todos 404 — esa sección del panel ("Configuración de Agentes") sigue sin un endpoint legible que se haya podido encontrar; tampoco hay evidencia de un scheduler nativo nuevo más allá del puente temporal que seguimos operando nosotros (no había ningún post pendiente en company 1 para probar un disparo real hoy). (3) **`POST /api/images/upload` — confirmado real y funcionando, pero con un flujo distinto al que trae armado el Image Generator AI**: acepta `multipart/form-data` con campo `file` (probado con un JPEG real) y devuelve `{"ok":true,"url":"https://api.growth54.com/storage/ai-images/{uuid}.jpg","bytes":N}` — es un paso de "subir archivo → te devuelvo URL" independiente, no algo atado a un post_id en el momento de subir. El workflow actual (`1rsqmFabW5PjnlHy`) hoy guarda directo la URL externa de Pollinations como `imagen_url` sin pasar por este endpoint — para aprovecharlo habría que agregar un paso que suba el binario (que el nodo `🌐 Pollinations: Generar Imagen` ya trae, con `responseFormat: file`) a este endpoint antes de guardar el post, y usar la URL que devuelve en vez de la de Pollinations directamente (mismo plan que ya se había anotado en el ítem 95, ahora con el endpoint real confirmado). Pendiente de confirmar con el usuario si se conecta ahora.
...
El día arrancó con un diagnóstico completo: repasé los 33 workflows de n8n, todos en success, y volví a analizar la API de G54 de punta a punta con el token de sesión, confirmando que varios pendientes de días anteriores seguían igual (ig_user_id null, timezone cruzado, el 404 de storage). Encontré algo nuevo: los posts ya tienen un campo pensado para guardar el ID real de Meta después de publicar, justo lo que se le había pedido a Walter hace días para que el Analytics pudiera consultar métricas de un post específico. Dejé a Distribution AI capturando y mandando ese ID en cada publicación, pero confirmé con pruebas directas que ninguno de los dos endpoints de guardado lo deja persistir todavía — la columna existe pero no es escribible, así que quedó listo de nuestro lado, esperando que G54 lo habilite.

De ahí pasé a revisar si el motor de tendencias del Ideas AI, separado del Strategist apenas ayer, realmente funcionaba, porque nunca se había confirmado con una tendencia real. Encontré que estaba roto por dos motivos: un resto de cuando se copió el código del Strategist, con una referencia a un nodo que no existe en este workflow nuevo, y un bug más sutil — Groq generaba el tema de tendencia en español pero la verificación contra las noticias reales buscaba en inglés, así que nunca encontraba cobertura suficiente para confirmarlo. Corregí los dos y lo confirmé en vivo con un tema real, verificado con 179 artículos recientes.

Ya en la tarde hice, a tu pedido, un análisis de usabilidad del Strategist y las Ideas desde la perspectiva de un cliente real, no solo técnico. Ahí fuimos ida y vuelta sobre varios puntos: qué hace exactamente el botón de descartar una idea, si el panel estaba mostrando bien las ideas nuevas (resultó que estabas viendo la empresa equivocada, no había ningún bug de nuestro lado), y cómo se podría retroalimentar el Analytics hacia las Ideas AI sin arriesgar pisar el número agregado de la cuenta con una consulta de post individual — quedó diseñado pero sin construir, a la espera de resolver lo del ID de Meta. De paso encontré que el dashboard de Métricas muestra los números mal, literalmente pegados como texto en vez de sumados, confirmé que el bug es del backend de G54 y no nuestro, y lo agregué a la lista de pendientes.

Seguí con una revisión completa del Editor de Posts — Content AI, el generador de imágenes y Distribution AI — y ahí salió el hallazgo más serio del día hasta ese momento: el mecanismo que debía revertir automáticamente un post a "aprobado" cuando fallaba la publicación nunca funcionó de verdad. Lo probé directo contra la API y confirmé que devuelve error de autenticación en cualquier post, siempre. En la práctica esto significa que cuando algo falla al publicar, el post queda atascado para siempre sin ningún aviso al cliente. Decidiste explícitamente no resolverlo metiendo una contraseña de administrador en ese workflow de producción, así que quedó documentado como pedido pendiente para Walter en vez de parcharlo.

Después de eso quisiste confirmar si los leads que genera el bot de comentarios de Facebook se estaban reflejando en el CRM de la plataforma, y ahí se abrió el resto del día. Investigué el CRM de G54 de punta a punta — resulta que existe un tablero Kanban real, separado del sistema de leads de WhatsApp que ya usábamos, y que ambos estaban completamente desconectados entre sí. Con tu autorización explícita para usar las credenciales de sesión esta vez, conecté tanto el bot de comentarios como el motor de ventas genérico al CRM. En el camino encontré un bug real en el motor de ventas — llevaba semanas construido pero nunca se había ejecutado de verdad, y tenía un error de sintaxis que rompía el guardado del historial de mensajes cada vez, corregido y confirmado. También encontré y borré un token real de Facebook que estaba hardcodeado como respaldo desde antes de esta sesión, y terminé de vincular los leads al CRM por su ID real de contacto en vez de por coincidencia de teléfono, para que borrar un lead borre también su tarjeta del CRM.

Ya con eso conectado, probaste comentar de verdad en una publicación real de Crating Express y no viste ninguna respuesta. Investigué la ejecución real y encontré que el bot sí generaba una respuesta correcta, pero Meta la rechazaba porque el token guardado en G54 era de tipo usuario, no de página — servía para leer datos pero no para publicar comentarios. Agregué un intercambio de token que resuelve esto en cada ejecución. Al probarlo con un comentario real nuevo salió un segundo problema distinto: al token le faltaban dos permisos específicos para comentar en Facebook. Confirmé cuáles con la API de Meta, me dijiste que sí tenías acceso para otorgarlos vos misma como administradora de la app, regeneraste el token con los permisos completos y lo llevé a la versión de larga duración.

Con el token ya arreglado probaste de nuevo, y ahí pasó lo más serio del día: el bot entró en un loop de auto-respuesta infinita, contestándose a sí mismo cada pocos segundos en una publicación real y pública. Lo desactivé en el momento en que avisaste. La causa era que el propio comentario del bot generaba un webhook nuevo de Meta, indistinguible de un comentario real, y el workflow no tenía ningún filtro para eso. Los cerca de 76 comentarios de spam que alcanzó a publicar en unos cuatro minutos ya habían sido borrados automáticamente por el propio sistema antispam de Facebook cuando fui a limpiarlos, así que no tuve que borrar nada a mano. Agregué dos guardias antes de reactivar: una que corta el flujo si el autor del comentario es la propia página, y otra que revisa si ya existe una respuesta previa de la página bajo ese comentario específico, por si Meta llega a reenviar el mismo evento — a pedido tuyo, porque la primera guardia sola no cubría ese caso. De paso corregí que el link de WhatsApp se estaba mandando duplicado en la respuesta y que faltaba configurar el link mismo. Reactivé y confirmamos juntas con un comentario real que ya respondía una sola vez, sin loop.

Al revisar el CRM con una captura tuya real encontramos tres cosas más: una tarjeta basura que había quedado del loop de más arriba (la borré), un teléfono inventado — el bot estaba usando el ID numérico de Facebook del usuario como si fuera un número de teléfono, cosa que se notó particularmente feo en la tarjeta basura porque ahí el "teléfono" terminó siendo el ID de la propia página — y un total en dólares en una columna del tablero que no correspondía a ningún dato real de los deals ahí adentro. Corregí el código para que ya no invente teléfonos y para que la relación entre lead y deal se haga por el ID real de contacto. El registro viejo que ya tenías abierto en pantalla no se pudo corregir porque el endpoint de G54 para editar un deal ignora esos dos campos en cualquier actualización — me pediste borrarlo directamente por ser una prueba, así que lo hice.

Para cerrar el día armamos el documento de pendientes para Walter, que en un principio organicé por prioridad y terminé reorganizando en tres categorías a tu pedido — qué necesita el panel, qué necesita el backend, y qué son solo consultas u opcionales — para que quede más fácil de repartir entre quien construya cada parte. Le agregaste seis puntos de interfaz que tenías anotados: el formulario para elegir el horizonte de la estrategia y escribir la dirección deseada, el contador visual de tiempo transcurrido, el aviso cuando una publicación falla, la justificación visible de por qué se generó cada idea, un botón para regenerar una idea sin perder su keyword, y el analytics agrupado por semana. Después de mostrártelo me corregiste tres cosas: separar en ese documento lo que el cliente necesita ver de lo que es solo uso interno de nuestros propios agentes, sacar el punto del timezone porque ya me habías dicho antes que era intencional, y sacar una nota final que ya no aplicaba. También me corregiste el nombre del documento, que le había puesto mal, y me pediste guardarlo directo en la rama master en vez de en la mía — lo hice con cuidado de no mezclar el resto de los cambios del día, solo ese archivo quedó en master. Cerré ahí, con todo pusheado y el documento confirmado.
