...
50. **G54 WhatsApp Engine migrado** (2026-05-27): workflow `eCOX3ogMjToxZsh9` migrado a G54 API — empresa y productos leen de G54; nombre del agente configurable vía campo `agent_name` en G54 WA Config; prompts, follow-up, welcome message, keywords listos para venir de `GET /api/n8n/companies/{id}/whatsapp/config` (endpoint pendiente de implementar en G54, `continueOnFail: true` con defaults); 0 tokens hardcodeados — todos usan `$vars`; leads siguen en Airtable hasta que G54 implemente endpoints WA. Archivo: `workflow-wa-engine-g54.json`. Variables n8n requeridas: `G54_N8N_TOKEN`, `G54_AGENT_TOKEN`, `G54_COMPANY_ID`, `AIRTABLE_TOKEN`, `FB_ACCESS_TOKEN`, `WA_PHONE_NUMBER_ID`.
