...
Segundo hallazgo, más urgente: revisando por qué los posts atascados de Orison (59, 61, 62) seguían en "programado" pese al fix del ítem 111, encontré que el puente de los 2 minutos los sigue trayendo en cada corrida (`to-publish` los sigue devolviendo, todavía con el `page_id`/`instagram_account_id` de Crating Express en vez de los propios de Orison — el cruce de datos del ítem 111 sigue sin corregir), Distribution AI intenta crear el container de Instagram, falla, y el nodo `♻️ G54: Revertir → Aprobado` —que debería devolver el post a "aprobado" para salir del loop— también falla con el mismo 401 Unauthenticated de la familia de rutas de sesión (`PUT /api/rrss/posts/{id}/status` es justo una de ellas). Resultado: desde que se activó el puente temporal el 12-jul, cada 2 minutos se está reintentando publicar los mismos 3 posts de Orison sin parar, fallando siempre igual, sin poder salir nunca del estado atascado — confirmado revisando el detalle de ejecuciones reales de hoy (`19931`, `19932`). No es dañino en el sentido de que no publica nada incorrecto, pero es un loop infinito de llamadas fallidas a la API de Meta y a G54 que no se va a resolver solo. Queda pendiente decidir con el usuario si conviene pausar el puente para company_id=6 mientras Walter no resuelva el cruce de credenciales y el 401 de esa ruta, para no seguir generando tráfico inútil.
