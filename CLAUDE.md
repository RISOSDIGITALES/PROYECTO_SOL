...
Ahí fue cuando se preguntó si no existía antes un "plan de contingencia" — un modo que hiciera las preguntas una por una sin depender de ninguna inteligencia artificial, y que al final avisara que un especialista contactaría por correo. Rastreé el historial completo de commits y encontré que sí existió, en la versión vieja de Marco basada en Airtable, y que se perdió sin querer cuando se migró todo a la arquitectura actual conectada a G54 — nadie lo eliminó a propósito, simplemente nunca se volvió a construir en la versión nueva. Lo reconstruí adaptado a los pasos actuales del flujo, lo probé localmente con los doce escenarios posibles antes de tocar producción, y de paso corregí el mismo bug de Gemini. La prueba final fue la mejor parte del día: con Groq y Gemini caídos al mismo tiempo en producción real, mandé una conversación completa simulada por el número de prueba y el sistema, sin ninguna inteligencia artificial funcionando, calculó y entregó una cotización real de $172 dólares solo con reglas simples. Cerré esa parte del día documentando un tercer hallazgo que quedó pendiente — un posible cierre prematuro de la conversación cuando el cliente responde negativamente a una pregunta — pero las pruebas de eso cayeron justo en la ventana donde se agotó la cuota de Groq, así que no pude confiarlas y quedó marcado como prioritario para retomar en cuanto la cuota se recupere, con el plan exacto de qué probar para no perder el hilo entre sesiones.

Ya más tarde retomé la revisión de G54, repasando de nuevo los pendientes que le habíamos dejado a Walter: ID de Instagram, los dos reportes de Analytics activos a la vez, borrado de leads de prueba, registro del Distribution AI en el panel, y una forma de generar ideas sueltas sin correr toda la estrategia. Solo se resolvió el bug visual de los botones; el resto sigue igual. Sobre las ideas sueltas, quedó claro que el agente ya existe y funciona, solo falta poder dispararlo desde el panel — decidimos esperar esa opción en vez de armar un atajo propio.

El resto del día lo dediqué a la nueva imagen de Marco. Revisé un research sobre estilos de personajes IA y quedó claro el criterio: un bot de chat como Marco debe verse claramente como IA, no como una persona real, para no sentirse engañoso; un estilo humano más cálido se reserva para contenido de marketing, no para el chat. Con eso decidido, usamos la imagen vieja de Marco (con el nombre "Alex" aún en el letrero) como referencia de ambientación y fui iterando una versión robot sentado en el mismo escritorio: primero salió muy genérico, después le agregamos traje azul, headset y un detalle de madera para conectarlo con la marca, y quedamos en darle una silueta más propia — cabeza angular, esquineros tipo cajón de madera, piernas con paneles de madera — para que no se sienta un robot cualquiera. Ese último ajuste quedó armado y pendiente de probar.
