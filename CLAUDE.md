...
- [ ] **🔴 PRIORITARIO — investigar y corregir el cierre prematuro con "no gracias" en cuanto Groq se recupere** (ver ítems 101-102, 105 del historial). Hallazgo sin confirmar: en una prueba real, el cliente dijo "no gracias" respondiendo a la recomendación de tipo de cajón, y Marco lo interpretó como que el cliente se despedía de toda la conversación, cerrando con "Quedo a tus órdenes cuando lo necesites" en vez de seguir preguntando lo que faltaba. La hipótesis de trabajo es que la conversación se clasificó como `tipo_flujo=consulta` en vez de `cotizacion` desde el principio (el cliente dijo "necesito embalar una lámpara" sin usar palabras como "cotizar"/"presupuesto"), y en modo consulta el cierre ante cualquier negativa tiene más sentido — pero **sigue sin confirmarse**. Dos intentos de prueba en limpio (2026-07-09) fallaron por la misma causa: la cuota diaria de Groq (100,000 tokens/día) se agota casi de inmediato con cualquier batería de pruebas, incluso después de parecer recuperada — un chequeo aislado puede pasar por un respiro momentáneo y dar falsa confianza. El segundo intento sí alcanzó a confirmar con IA real (antes de caer en Modo Contingencia) que "necesito embalar una lámpara" se clasifica como `consulta`, no `cotizacion` — consistente con la hipótesis, pero falta la segunda mitad de la prueba (cómo se comporta el cierre cuando SÍ se clasifica como `cotizacion` desde el inicio) para confirmar del todo. **Antes de tocar nada:** cuando se reintente, usar máximo 1-2 conversaciones cortas por sesión de prueba (no 4 en batch) para no repetir el agotamiento, y verificar turno por turno que la ejecución use IA real (`groq=OK` en la data de ejecución) antes de confiar en el resultado — no asumir que porque el primer mensaje respondió bien, el resto de la conversación también lo hizo.
...
Retomé la revisión de G54 justo donde la había dejado el día anterior, sin esperar mucho cambio de un día para el otro: repasé de nuevo cada punto que le habíamos dejado a Walter en el documento de pendientes — si el ID de Instagram ya estaba disponible, si ya se podían tener los dos reportes de Analytics activos al mismo tiempo, si había forma de borrar leads de prueba, si el Distribution AI ya aparecía en el panel de agentes, y si ya existía una manera de generar una idea de contenido suelta sin correr todo el proceso de estrategia completo. De todo eso, solo se resolvió el bug visual de los botones sin estilo, así que lo dimos de baja de la lista. Todo lo demás sigue exactamente igual, incluyendo que el Distribution AI todavía no aparece registrado en el panel. Sobre lo de generar una idea suelta, aclaramos algo importante: el agente que las genera ya existe y funciona, lo que falta es una forma de dispararlo manualmente desde la interfaz — decidimos esperar a que esa opción llegue del lado del panel en vez de armar un atajo por nuestra cuenta que después haya que mantener aparte.

Después me puse a trabajar en la nueva imagen de Marco. Analicé un research que me compartieron sobre distintos estilos de personajes de inteligencia artificial — desde mascotas tipo robot hasta avatares humanos muy realistas, como los que usan las plataformas de presentadores virtuales — para pensar cuál le conviene más a Marco específicamente. Mi conclusión fue separar los dos usos: para un bot de conversación como Marco, lo más honesto es que se vea claramente como una inteligencia artificial y no como una persona real, para no dar la sensación de que el cliente fue engañado cuando descubre que en realidad habla con un bot; un personaje humano más cálido queda mejor reservado para otro tipo de contenido, como videos de marketing, no para el chat en sí.

Con esa dirección clara, retomamos la imagen que ya existía de Marco (todavía con el nombre viejo, Alex, en el letrero) como referencia de ambientación — la oficina, el skyline de Miami, los colores de marca — y fui armando paso a paso una versión nueva en forma de robot sentado en el mismo escritorio. El primer resultado fue un robot genérico, sin nada que lo conectara con la identidad anterior de Marco, así que lo fuimos ajustando: primero le agregué el traje azul marino, el headset de call center y un detalle de madera como guiño al negocio de los cajones, lo cual ya se sintió más parecido al Marco original. Como igual seguía sintiéndose como "cualquier robot de inteligencia artificial", quedamos en ir un paso más allá y darle una silueta propia — cabeza con forma más angular en vez de casco redondo genérico, esquineros metálicos como los de un cajón de madera en lugar de orejas comunes, y piernas con paneles de madera que combinen con el resto del cuerpo — para que el diseño se sienta hecho a la medida del negocio y no una mascota genérica. Quedó ese último ajuste armado y pendiente de probarse.

---

### 2026-07-09 (Jueves)

El día arrancó pidiendo un diagnóstico general de los 41 workflows que tenemos corriendo en n8n — última ejecución y resultado de cada uno. Buena noticia de entrada: ninguno estaba fallando. Pero salieron tres workflows activos que nunca habíamos documentado (un procesador de medios para que Marco pueda leer audios e imágenes que todavía no está conectado al motor principal, un wrapper del Analytics Semanal, y uno para mandar respuestas manuales de WhatsApp desde el panel), un ID desactualizado del Sales AI Motor en la documentación, y sobre todo confusión sobre dos workflows viejos — "00 Orquestador" y "04 Cotizador" — que parecían formar parte de la arquitectura de Marco pero llevaban tiempo inactivos. Investigué a fondo y expliqué que son versiones predecesoras que quedaron ahí sin usarse, no algo que estuviera roto.

Ahí fue donde se puso serio el día. Al mencionar que "04 Cotizador" estaba desconectado, el susto fue inmediato — con razón, porque coincidía con la sospecha de que Marco no estaba dando cotizaciones. Antes de tocar nada me puse a investigar con calma: probé la API externa del cotizador cinco veces con datos distintos y respondió perfecto las cinco, así que no era un problema de esa API. Terminé auditando 225 conversaciones reales de las últimas dos semanas y confirmé algo grave: ninguna había llegado nunca a completar una cotización. La causa real no tenía nada que ver con el cotizador desconectado — era un bug de una sola línea en el subworkflow que reconstruye los datos de la conversación entre mensajes. Cuando un cliente respondía que no necesitaba protección extra, ese "no" se guardaba bien en el momento pero se perdía apenas llegaba el siguiente mensaje, porque el código trataba cualquier valor "false" como si no existiera. El cliente quedaba atrapado respondiendo la misma pregunta para siempre sin darse cuenta. Lo corregí, lo probé con una conversación simulada antes y después del cambio para confirmarlo, y quedó resuelto.

Esa misma prueba de verificación destapó un segundo problema: cuando un cliente decía directamente qué tipo de embalaje quería (por ejemplo "jaula por favor") en vez de esperar a que Marco se lo recomendara, la inteligencia artificial lo ignoraba y repetía su propia recomendación en loop. En vez de parchear solo ese caso puntual, armé una solución más general: ajusté la instrucción para que la IA reconozca cualquier preferencia que el cliente ya haya dado, y además agregué una red de seguridad por reglas que reconoce las variantes en español e inglés de los distintos tipos de embalaje aunque la IA falle en detectarlo, y de paso corrige el formato del dato guardado para que siempre coincida exactamente con lo que espera la API de cotización. Antes de subir nada a producción lo probé en una simulación local en mi máquina con una docena de casos distintos, y recién después lo probé en vivo con tres conversaciones completas — en español dejando que el cliente se adelante, en español confirmando la recomendación del bot, y en inglés — las tres funcionaron bien.

Justo en esas pruebas la cuota diaria gratuita de Groq (la inteligencia artificial principal de Marco) se agotó, en buena parte por el volumen de mis propias pruebas del día. Eso destapó algo más serio: el respaldo de Gemini, que debería activarse automáticamente cuando Groq falla, también estaba roto, por el mismo tipo de bug de una variable de n8n que esta instancia ya no soporta desde hace semanas. Cuando ambas inteligencias fallan al mismo tiempo, Marco se queda repitiendo un mensaje fijo sin poder avanzar la conversación — así quedó claro por qué de repente el bot se veía "trabado" en una de mis pruebas en inglés, que en un principio pensé que era un problema de idioma y en realidad era este apagón de las dos IA a la vez.

Ahí fue cuando se preguntó si no existía antes un "plan de contingencia" — un modo que hiciera las preguntas una por una sin depender de ninguna inteligencia artificial, y que al final avisara que un especialista contactaría por correo. Rastreé el historial completo de commits y encontré que sí existió, en la versión vieja de Marco basada en Airtable, y que se perdió sin querer cuando se migró todo a la arquitectura actual conectada a G54 — nadie lo eliminó a propósito, simplemente nunca se volvió a construir en la versión nueva. Lo reconstruí adaptado a los pasos actuales del flujo, lo probé localmente con los doce escenarios posibles antes de tocar producción, y de paso corregí el mismo bug de Gemini. La prueba final fue la mejor parte del día: con Groq y Gemini caídos al mismo tiempo en producción real, mandé una conversación completa simulada por el número de prueba y el sistema, sin ninguna inteligencia artificial funcionando, calculó y entregó una cotización real de $172 dólares solo con reglas simples. Cerré el día documentando un tercer hallazgo que quedó pendiente — un posible cierre prematuro de la conversación cuando el cliente responde negativamente a una pregunta — pero las pruebas de eso cayeron justo en la ventana donde se agotó la cuota de Groq, así que no pude confiarlas y quedó marcado como prioritario para retomar en cuanto la cuota se recupere, con el plan exacto de qué probar para no perder el hilo entre sesiones.
