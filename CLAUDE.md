...
113. **Análisis completo API G54 2026-07-13 — causa raíz real del bloqueo de endpoints "de panel" encontrada, y loop activo de reintentos fallidos descubierto en vivo** (2026-07-13): repasando de punta a punta los pendientes (Éxodo del 08-jul, hallazgos del 12-jul) contra la API real. Primero, el hallazgo más importante: `GET /sanctum/csrf-cookie` en `apigrowth.mdarthurdigital.com` responde con cookies `XSRF-TOKEN` y `growth54-session` — esto confirma que los endpoints "de panel" que vengo topando desde el 12-jul (`/api/social-channels`, `PUT/DELETE /api/rrss/posts/{id}`, `POST /api/images/upload`, `GET /api/whatsapp/conversations`, `DELETE /api/whatsapp/leads/{id}`, y también el más viejo `PUT /api/rrss/posts/{id}/status` que usa Distribution AI) no están simplemente pidiendo "un token distinto" — están montados bajo el middleware de sesión de Laravel (Sanctum SPA / cookies), el mismo que usa el panel admin cuando un humano hace login en el navegador. Ningún Bearer token, sea cual sea, puede autenticar ahí — es una arquitectura de auth completamente distinta a la de `/api/n8n/*` y `/api/agent/*` (que sí son token-guarded y por eso siempre funcionaron). Esto no es que "nos dieron el token equivocado": para que un agente n8n pueda llamar estas rutas, Walter necesita moverlas al grupo de rutas con guard de token (como las demás rutas de agente) o emitir un Personal Access Token de Sanctum real para uso de agentes — dos soluciones concretas y accionables en vez del "no funciona" de ayer.
