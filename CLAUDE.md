...
El día arrancó con la novedad de que ya se podía registrar el webhook del Distribution AI en el panel nuevo de G54 ("Configuración de Agentes"), así que lo primero fue confirmarlo y probarlo. Pasó la prueba desde el panel, pero quedaba la pregunta real: si al llegar la hora programada de un post, G54 realmente llama solo al agente. Al principio saqué una conclusión apurada después de monitorear solo unos minutos sin ver nada — se me hizo notar con razón que el ciclo de G54 podía tardar más, así que dejé un monitoreo de casi media hora corriendo antes de sacar cualquier conclusión. Confirmado con ese tiempo más largo y una segunda prueba limpia: G54 todavía no tiene ningún disparador automático que llame al agente cuando llega la fecha programada — el webhook en sí funciona perfecto, pero nada lo dispara solo. Diseñamos entre los dos una arquitectura mejor a futuro (una "alarma exacta" con un Wait de n8n en vez de estar consultando cada tanto), pero se dejó anotada para más adelante porque no era urgente ese mismo momento.

Eso cambió de golpe cuando surgió que se vienen demostraciones pronto y hace falta que las publicaciones programadas salgan sí o sí. Activé la solución temporal que habíamos diseñado como respaldo: un workflow que cada 2 minutos revisa las empresas activas en G54 y llama al Distribution AI por cada una. Quedó confirmado funcionando en vivo, con la aclaración en el CLAUDE.md de que es explícitamente temporal y hay que quitarlo en cuanto esté lista la solución de la alarma exacta.

De ahí revisé el estado general de Marco para asegurarme de que todo seguía respondiendo bien antes de tocar nada más delicado, y aproveché para conectar el subworkflow de Procesador de Medios (el que lee imágenes, audios y documentos que un cliente manda por WhatsApp) al motor principal — algo que estaba construido desde hace tiempo pero nunca se había enlazado. Fui con mucho cuidado porque Marco está en producción: mapeé todo el flujo de datos existente antes de tocar cualquier nodo, hice los cambios de forma aditiva (nada del camino de texto normal se modificó), y probé varias conversaciones de texto completas antes y después para confirmar cero regresión. En el proceso encontré y corregí dos bugs reales dentro del subworkflow de medios que nunca se habían detectado porque nunca había corrido ni una sola vez: una referencia a un dato que en realidad ya no estaba disponible en ese punto del flujo, y el mismo problema de credencial mal configurada que ya había aparecido antes en el Distribution AI. Quedó un tercer problema sin resolver — un nodo que debería repartir el archivo según su tipo (audio, imagen, PDF, office) no está enrutando ningún item — lo dejé con una red de seguridad para que Marco nunca se quede sin responder por esto, pero sin la causa raíz encontrada todavía.

Aproveché que ya estaba ahí para revisar el mecanismo de escalada a un especialista, algo que ya existía a medias: todo el código de "qué hacer cuando se escala" — guardar el tema, notificar al vendedor — ya funcionaba, pero nunca se disparaba porque el prompt del bot nunca le decía a la IA que ese campo existiera. Lo conecté agregando la instrucción y el campo al formato que la IA tiene que devolver, con la regla de que primero pide el correo si no lo tiene y recién ahí marca la escalada. También lo dejé listo para el caso nuevo que se había pedido: si Marco no logra procesar un archivo que el cliente mandó, use la misma escalada en vez de quedarse sin saber qué decir. Confirmado con una conversación real de varios turnos que terminó con un correo real al vendedor.

Después de eso llegaron capturas de pantalla de funciones nuevas en la API de G54 — un endpoint de canales sociales con configuración de estilo de imagen por plataforma, edición y borrado de posts, y subida de imágenes. Al revisar si nuestro generador de imágenes podía leer esa configuración de estilo por canal, mi primera revisión dio que no había forma de leerlo con las credenciales que tenemos — hasta que me señalaron, con razón, que si el panel ya lo guarda entonces tiene que poder leerse de algún lado, y que no me quedara en esa conclusión sin agotar todas las variantes. Volví a probar de más formas — distintos headers, distintas rutas, distintos formatos de token — y la conclusión se sostuvo: esos endpoints nuevos (igual que uno que llegó después para conversaciones y borrado de leads de WhatsApp) piden un tipo de credencial distinta a la que tenemos, algo así como un token de sesión del panel, no el AGENT_TOKEN de siempre. Probé capturar ese token desde el navegador con las herramientas de desarrollador para tener algo real con qué probar, y con un mensaje que llegó del equipo de Walter sobre el borrado de leads probé varias variantes del token que se mencionaron — todas devolvieron el mismo rechazo. Quedó claro que esto no es nada que se pueda resolver de nuestro lado; hace falta o el token real de sesión o que el backend acepte el AGENT_TOKEN en esas rutas también.

En medio de esa investigación surgió algo más urgente: un post recién programado no se estaba publicando pese a que el puente temporal de los 2 minutos ya estaba corriendo. Al meterme a fondo encontré un bug bastante serio en el Distribution AI, agazapado ahí desde que se construyó: dos nodos de código estaban escritos de una forma que en n8n solo procesa el primer item de la lista en vez de recorrerlos todos. En la práctica esto significaba que si una empresa tenía más de un post pendiente de publicar, el workflow solo tocaba siempre el más viejo y los demás quedaban completamente ignorados en cada corrida, sin importar cuántas veces se ejecutara. Lo corregí para que recorra la lista completa, y confirmé en vivo que ahora sí procesa todos los posts pendientes de una empresa, no solo el primero.

Al cierre del día se me pidió leer un documento de aclaración que había guardado sobre el tema del token — pero al intentar abrirlo resultó que el archivo guardado solo traía el marco exterior de la página, sin el contenido real (el texto vivía en una parte de la página que no se guardó). Quedó pendiente que se comparta ese contenido de otra forma — copiando el texto directo o con una captura de pantalla — para poder leerlo, y con eso se cerró la jornada.
