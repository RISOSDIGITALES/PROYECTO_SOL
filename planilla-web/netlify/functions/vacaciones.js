...
exports.handler = async (event, context) => {
  ...
  try {
    if (method === 'GET') {
      ...
      const records = await atAll('vacaciones', opts);
      return resp(200, records.map(r => ({ id: r.id, ...r.fields })));
    }

    if (method === 'POST') {
      const fields = JSON.parse(event.body);
      const data = await atFetch('vacaciones', {
        method: 'POST',
        body: { records: [{ fields: { ...fields, Estado: fields.Estado || 'Aprobada' } }] },
      });
    ...
    if (method === 'DELETE') {
      ...
      await atFetch(`vacaciones/${id}`, { method: 'DELETE' });
